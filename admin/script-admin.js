...
// Check if the user has previously set a mode preference
const modePreference = localStorage.getItem('mode');

if (modePreference === 'dark') {
    document.body.classList.add('dark');
    switchMode.checked = true; // Make sure the switch is in the correct position
}

switchMode.addEventListener('change', function() {
    if (this.checked) {
        document.body.classList.add('dark');
        localStorage.setItem('mode', 'dark'); // Store the preference
    } else {
        document.body.classList.remove('dark');
        localStorage.setItem('mode', 'light'); // Store the preference
    }
});
